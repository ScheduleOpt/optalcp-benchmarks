...
import { parse, type ParseResult } from '../tsp/parsetsp.mjs';
...
function defineModel(filename: string): CP.Model {
  const { nbNodes, transitionMatrix, demands, capacity, depots, hasDirectionSymmetry }: ParseResult =
    parse(filename, {checkTriangularInequality, visitDuration, forceCeil, checkDirectionSymmetry});
  ...
  if (nbVehicles === 0) {
    const match = filename.match(/k(\d+)\.vrp(\.gz)?$/);
    if (match !== null)
      nbVehicles = Number.parseInt(match[1]);
    else {
  ...
  const horizon = maxDistance * (nbNodes + nbVehicles);
  ...
  const customerMatrix = transitionMatrix.slice(1).map(row => row.slice(1));
  ...
  const model = new CP.Model(makeModelName('cvrp', filename));
  ...
  const vehicleUsage: CP.IntExpr[] = [];

  for (let v = 0; v < nbVehicles; v++) {
    // Visits done by the vehicle v:
    const myVisits = Array.from({ length: nbCustomers }, (_, i) =>
    ...
    const last = model.intervalVar({ length: 0, name: `last_${v + 1}`, end: [0, horizon] });
    for (let i = 0; i < nbCustomers; i++) {
      ...
      myVisits[i].startMin = transitionMatrix[0][i + 1];
    ...
    const used = model.sum(myVisits.map((itv, i) => itv.presence().times(demands[i + 1])));
    ...
    if (hasDirectionSymmetry && breakDirectionSymmetry) {
      ...
      model.enforce(timeOfMaxServedCustomer.times(2).le(last.end()));
    }
  }
  ...
  // remaining vehicles.
  const totalDemand = demands.slice(1).reduce((a, b) => a + b, 0);
  model.enforce(model.sum(vehicleUsage).eq(totalDemand));
...
forcedNbVehicles = getIntOption("--nbVehicles", forcedNbVehicles, restArgs);
...
objective = getStringOption("--objective", objective, restArgs);
if (objective !== "makespan" && objective !== "path") {
  console.error("Invalid value for --objective. Can only be 'makespan' or 'path'");
  process.exit(1);
}
